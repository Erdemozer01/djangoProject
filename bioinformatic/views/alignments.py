...
def MultipleSeqAlignment(request):
    ...
    request.user.id = uuid.uuid4().hex[:8]
        ...
        if form.is_valid():
            ...
            if method == "MUSCLE":
                try:
                    ...
                    output_file = os.path.join(BASE_DIR, 'bioinformatic', 'files', f'{user_id}.fasta')
                    align_file = os.path.join(BASE_DIR, 'bioinformatic', 'files', f'{user_id}.aln')
